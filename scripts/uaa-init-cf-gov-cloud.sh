...
uaac target https://stargate-uaa.apps.vi2e.io --skip-ssl-validation

uaac token client get admin -s adminsecret

uaac client add stargate --name stargate --scope stargate.read,stargate.write,openid,profile,email,address,phone -s stargatesecret --authorized_grant_types authorization_code,refresh_token,client_credentials,password --authorities uaa.resource --redirect_uri https://stargate.apps.vi2e.io/\*\*,http://stargate.apps.vi2e.io/\*\*,http://localhost:8081/\*\*

uaac user add user1 -p user1 --emails user1@example.com
