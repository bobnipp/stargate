...
# Run like this: STARGATE_HOST=<your host name here> ./scripts/uaa-init-aws.sh
...
# get token
get_token_response=$(curl "http://$STARGATE_HOST:8080/uaa/oauth/token" -s -i -X POST \
...
create_client_response=$(curl "http://$STARGATE_HOST:8080/uaa/oauth/clients" -s -i -X POST \
...
# create user 'user1'
create_user_response=$(curl "http://$STARGATE_HOST:8080/uaa/Users" -s -i -X POST \
    -H 'Accept: application/json' \
    -H "Authorization: Bearer $TOKEN" \
    -H 'Content-Type: application/json' \
    -d '{
...
create_read_group_response=$(curl "http://$STARGATE_HOST:8080/uaa/Groups" -s -i -X POST \
    -H 'Content-Type: application/json' \
    -H "Authorization: Bearer $TOKEN" \
    -d '{
...
create_write_group_response=$(curl "http://$STARGATE_HOST:8080/uaa/Groups" -s -i -X POST \
    -H 'Content-Type: application/json' \
    -H "Authorization: Bearer $TOKEN" \
    -d '{
